...
class MonoRotDistToGoal(RotDistToGoal):
    cls_uuid: "mono_rot_dist_to_goal"
    ...
    def _get_uuid(*args, **kwargs):
        return MonoRotDistToGoal.cls_uuid
...
class MonoDistToGoal(DistToGoal): 
    cls_uuid: str = "mono_dist_to_goal"
    def __init__(self, *args, config, **kwargs):
        self._config = config
        super().__init__(*args, config=config, **kwargs)
    ...
    @staticmethod
    def _get_uuid(*args, **kwargs):
        return MonoDistToGoal.cls_uuid
...
# Mono Navigation to Position Success # 
@registry.register_measure
class MonoNavToPosSucc(NavToPosSucc):
    uuid: str = "mono_nav_to_pos_succ"
    def update_metric(self, *args, episode, task, observations, **kwargs):
        dist = task.measurements.measures[MonoDistToGoal.cls_uuid].get_metric()
        self._metric = dist < self._config.success_distance
    
    def reset_metric(self, *args, task, **kwargs):
        task.measurements.check_measure_dependencies(
            self.uuid,
            [MonoDistToGoal.cls_uuid],
        )
        self.update_metric(*args, task=task, **kwargs)

    def _get_uuid(self, *args, **kwargs):
        return MonoNavToPosSucc.uuid

# Did the robot na
@registry.register_measure
class DidMonoNavToPosSuccess(Measure):
    ...
    @staticmethod
    def _get_uuid(*args, **kwargs):
        return DidMonoNavToPosSuccess.cls_uuid
    ...
    def reset_metric(self, task, *args, **kwargs):
        ...
        task.measurements.check_measure_dependencies(
            self.uuid,
            [MonoNavToPosSucc.cls_uuid],
        )
        self.update_metric(task, *args, **kwargs)
    
    def update_metric(self, task, observations, *args, **kwargs):
        nav_to_obj_succ = task.measurements.measures[MonoNavToPosSucc.cls_uuid].get_metric()
...
@registry.register_measure
class DidMonoNavToObjSuccess(Measure):
    ...
    @staticmethod
    def _get_uuid(*args, **kwargs):
        return DidMonoNavToObjSuccess.cls_uuid
    ...
    def reset_metric(self, task, *args, **kwargs):
        ...
        task.measurements.check_measure_dependencies(
            self.uuid,
            [MonoNavToObjSuccess.cls_uuid],
        )
        self.update_metric(task, *args, **kwargs)
    
    def update_metric(self, task, observations, *args, **kwargs):
        nav_to_obj_succ = task.measurements.measures[MonoNavToObjSuccess.cls_uuid].get_metric()
...
@registry.register_measure
class MonoNavToObjReward(RearrangeReward):
    cls_uuid: str = "mono_nav_to_obj_reward"

    @staticmethod
    def _get_uuid(*args, **kwargs):
        return MonoNavToObjReward.cls_uuid
...
@registry.register_measure  
class MonoNavToObjSuccess(NavToObjSuccess):
    cls_uuid: str = "mono_nav_to_obj_success"
    
    @staticmethod
    def _get_uuid(*args, **kwargs):
        return MonoNavToObjSuccess.cls_uuid
